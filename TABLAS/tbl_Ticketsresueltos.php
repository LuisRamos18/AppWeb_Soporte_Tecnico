...
T.motivo, T.statu,(select usuario from usuarios where id_usuario=T.id_tecnico) as 'Atiende',
 ...
 inner join departamentos D on D.id_departamento=U.id_departamento inner join historial H on H.id_ticket=T.id_ticket 
 where H.estado='Resuelto' and T.statu='Resuelto';";
...
foreach($listpro as $pro) 
{
    ...
    <td>'.$pro["Atiende"].'</td>
    <td>
    <center><button type="button" class="btn btn-info" data-bs-toggle="modal" data-bs-target="#modalResuelto" onclick="buscarsol('.$pro["id_ticket"].');"><i class="material-icons text-sm my-auto me-1"</i>check</button><center></td>
